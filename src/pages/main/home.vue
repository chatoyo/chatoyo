...
const showChannelList = () => {
  homeResponsiveDisplay.value = false;
  channelSelected.value = undefined;
};
const homeResponsiveDisplay = ref(true);
const listResponsiveDisplay = computed(() => !homeResponsiveDisplay.value && !channelSelected.value);
</script>

<template>
  <div class="home-page flex flex-row w-full">
    <div class="channel-list" :class="{ hidden: !listResponsiveDisplay }">
      <channel-list
        v-model:selected="channelSelected"
        :data="channels"
        title="频道"
        subtitle="Channel"
        @back-home="homeResponsiveDisplay = true"
      />
    </div>

    <div class="main-container" :class="{ hidden: listResponsiveDisplay }">
      <channel-detail v-if="channelSelected" :channel="channelSelected" :messages="chatLog" />
      <home-card v-else />
      <div class="md:hidden" @click="showChannelList">打开频道列表</div>
    </div>
  </div>
</template>
